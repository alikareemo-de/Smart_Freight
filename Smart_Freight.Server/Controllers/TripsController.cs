...
public class TripsController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (status is TripStatus.Done)
        {
            return BadRequest(new { message = "Done status can only be set via the done endpoint." });
        }
        ...
        return Ok(MapTripSummary(trip));
    }

    [HttpPost("{id:guid}/done")]
    [Authorize(Roles = "Driver")]
    public async Task<ActionResult<TripSummaryResponse>> MarkTripDone(Guid id, CancellationToken cancellationToken)
    {
        var trip = await _dbContext.Trips
            .Include(item => item.Truck)
            .Include(item => item.Driver)
        ...
        trip.DoneAt = DateTimeOffset.UtcNow;
