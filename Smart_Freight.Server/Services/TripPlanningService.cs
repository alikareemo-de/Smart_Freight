...
public class TripPlanningService : ITripPlanningService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requestedStatus = TripStatus.Waiting;
        if (!string.IsNullOrWhiteSpace(request.Status))
        {
            if (!Enum.TryParse<TripStatus>(request.Status, true, out requestedStatus)
                || requestedStatus == TripStatus.Done)
            {
                throw new InvalidOperationException("Trip status must be Waiting, Active, or Cancelled.");
            }
        }
