...
import { useMarkTripDone, useTrip, useUpdateTripStatus, useUpdateTripStopStatus } from '../hooks/trips';
import type { TripStop } from '../types/trip';

const editableTripStatuses = ['Waiting', 'Active', 'Cancelled'] as const;
...
const TripDetailsPage = () => {
    ...
    const markDoneMutation = useMarkTripDone(id ?? '');
    ...
    const canMarkDone = auth?.role === 'Driver' && data.status !== 'Done';
            ...
            <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                    <Paper variant="outlined" sx={{ p: 3 }}>
                        ...
                        <Typography>{data.status}{data.doneAt ? ` · ${new Date(data.doneAt).toLocaleString()}` : ""}</Typography>
                        <Typography fontWeight={600} mt={2}>
                            Driver
                        </Typography>
                        ...
                        {canManageTrip && (
                            <Box mt={3}>
                                ...
                                >
                                    {editableTripStatuses.map((status) => (
                    ...
                    <Button
                        ...
                        onClick={async () => {
                            try {
                                ...
                                enqueueSnackbar('Trip marked as done.', { variant: 'success' });
                            } catch (error: any) {
                                const message = error?.response?.data?.message ?? 'Unable to mark trip as done.';
                                enqueueSnackbar(message, { variant: 'error' });
                            }
                        }}
                    >
