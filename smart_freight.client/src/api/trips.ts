...
export const updateTripStatus = async (id: string, payload: TripStatusUpdateRequest) => {
    const { data } = await httpClient.put<TripSummary>(`/trips/${id}/status`, payload);
    return data;
};

export const markTripDone = async (id: string) => {
    const { data } = await httpClient.post<TripSummary>(`/trips/${id}/done`);
    return data;
};
